...
export async function POST(request: NextRequest) {
  try {
    ...
    // 🔥 CORRIGIDO: Buscar telefone do array de passageiros
    const telefone = reserva.passageiros?.[0]?.telefone || reserva.passageiro?.telefone;
    ...
    const templateData = {
      ...
      nomePassageiro: reserva.passageiro?.nome || reserva.passageiros?.[0]?.nomeCompleto || '',
      nomePassageiroUpper: (reserva.passageiro?.nome || reserva.passageiros?.[0]?.nomeCompleto || '').toUpperCase(),
      documentoPassageiro: reserva.passageiro?.documento || reserva.passageiros?.[0]?.docNumero || '',
    ...
    if (!pdfshiftResponse.ok) {
      const errorText = await pdfshiftResponse.text().catch(() => '');
