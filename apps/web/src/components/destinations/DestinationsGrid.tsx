  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="relative h-40 w-full overflow-hidden">
        ...
        <div className="absolute inset-0 animate-[shimmer_1.6s_infinite] bg-[linear-gradient(100deg,transparent_40%,rgba(255,255,255,.5)_50%,transparent_60%)] bg-[length:200%_100%]" />
      </div>
...
function PopularCard({ c }: { c: PopularConn }) {
  ...
  return (
    <article
      className={cx(
        "group overflow-hidden rounded-2xl border border-[#E6E8EF] bg-white",
        "shadow-[0_6px_22px_rgba(16,24,40,0.06)] transition-transform duration-300",
    ...
    >
      ...
      <div className="space-y-3 p-4">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-3">
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-200 px-2.5 py-2 text-center text-[14px] outline-none focus:border-sky-400"
              />
              <button
                type="button"
        ...
        <a
          ...
          className={cx(
            "block rounded-2xl bg-[#0B74A5] px-4 py-3 text-center text-[15px] font-bold text-white",
...
export default function DestinationsGrid() {
  const [data, setData] = useState<PopularConn[] | null>(null);
  ...
  return (
    <section className="relative bg-white">
      <div className="mx-auto max-w-7xl px-4 md:px-8 py-12 md:py-16">
        {/* Cabeçalho com Exo_2, igual HowItWorks */}
        <div className="mb-6 md:mb-8 flex items-end justify-between">
          <div>
            <h2 className={`${exo2.className} text-2xl md:text-3xl font-extrabold text-navy`}>
              Destinos <span className="text-accent">mais populares</span>
            </h2>
            <p className="mt-2 text-ink/70 text-sm">Inspire-se e partiu viajar.</p>
          </div>
        </div>
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6">
            {(data ?? []).map((c, i) => (
              <PopularCard key={`${c.from.code}-${c.to.code}-${i}`} c={c} />
            ))}
          </div>
        )}
      </div>

      {/* glows (harmoniza com a home) */}
...
/* Tailwind extra (coloque no globals.css caso queira o shimmer)
@keyframes shimmer {
